...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Continued Pre-Training with TinyFT (YAML-First Configuration)",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
        epilog="""
Examples:
  # Basic usage with config file
  python train_pretrain.py --config configs/base_lora_cpt.yaml
  
  # Quick override for experimentation
  python train_pretrain.py --config configs/base_lora_cpt.yaml --output_dir ./cpt_experiment --run_name "domain_adapt"
  
  # Evaluation only
  python train_pretrain.py --config configs/trained_cpt_model.yaml --eval_only
  
  # Resume training
  python train_pretrain.py --config configs/cpt_experiment.yaml --resume_from_checkpoint ./outputs/checkpoint-1000
        """
    )
...
def validate_config(config: dict) -> None:
    ...
    required_fields = [
        ...
        'batch_size', 'num_epochs', 'output_dir', 'text_column'
    ]
...
def print_config_summary(config: dict) -> None:
    ...
    print("🚀 TinyFT Continued Pre-Training")
    print("=" * 50)
    ...
    print(f"Max Length: {config.get('max_seq_length', 'Unknown')}")
...
def main():
    ...
    try:
        ...
        loggers = setup_logging(
            ...
            project_name=config.get('project_name', 'tinyft_cpt'),
        ...
        logger.info("Starting TinyFT Continued Pre-Training")
        ...
        logger.info("Initializing TinyFT trainer for continued pre-training")
        ...
        if config.get('eval_only'):
            ...
            logger.info("Starting continued pre-training from scratch")
        
        print("✅ Continued pre-training completed successfully!")
        logger.info("Continued pre-training completed successfully")
